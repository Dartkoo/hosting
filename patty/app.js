...
async function loadEntertainmentThemes(){
  ...
  if(!entertainmentThemes.some(x=>x.id===activeEntertainmentTheme && x.active)) activeEntertainmentTheme=entertainmentThemes.find(x=>x.active)?.id||null;
}
...
$('viewerImage').onclick=()=>setViewerScale(viewerScale===1?2:1);$('viewerZoomIn').onclick=()=>setViewerScale(viewerScale+0.5);$('viewerZoomOut').onclick=()=>setViewerScale(viewerScale-0.5);$('viewerZoomFit').onclick=()=>setViewerScale(1);$('viewerImage').addEventListener('wheel',e=>{if($('photoViewer').classList.contains('hidden'))return;if(viewerScale>1){return}if(e.ctrlKey){e.preventDefault();setViewerScale(viewerScale+(e.deltaY<0?0.25:-0.25))}},{passive:false});$('viewerImage').addEventListener('touchstart',e=>{if(e.touches.length===2){pinchStartDistance=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);pinchStartScale=viewerScale}},{passive:true});$('viewerImage').addEventListener('touchmove',e=>{if(e.touches.length===2&&pinchStartDistance){e.preventDefault();const d=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);setViewerScale(pinchStartScale*(d/pinchStartDistance))}},{passive:false});$('viewerImage').addEventListener('touchend',()=>{pinchStartDistance=0},{passive:true});
